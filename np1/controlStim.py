...
class CntrlStim(Stimulator):
    ...
    trapezoidal_prueba = [1,2,3,4,5,6,7,8,9,9,9,9,9,9,9,9,9,9,9,8,7,6,5,4,3,2,1]
    ...
    def estimular(self):
        ...
        super().wait_for_packet(super().STARTCHANNELLISTMODEACK)
        # Watchdog
        super().send_packet(super().WATCHDOG)
        super().send_packet(super().WATCHDOG)
        time.sleep(CntrlStim.t_a)
    ...
    def sendTrapecio(self, canal, vector= trapezoidal_prueba):
        ...
        for c in vector:
            self.chanels[canal-1]['current'] = c
            self.estimular()
